...
describe("cache-debug", async () => {
  ...
  describe("debug scenarios", () => {
    it("should create a use and and query it two time first is cache miss second is hit, also clear by key and global", async () => {
      ...
      const key = "key";
      const ttl = "1 second";
      const cacheMissRegExp = /\[simple-cache-mongoose\] GET '.*?' - MISS/;
      const cacheHitRegExp = /\[simple-cache-mongoose\] GET '.*?' - HIT/;
      const cacheSetRegExp =
        /\[simple-cache-mongoose\] SET '.*?' - ttl: \d+ ms/;
      const cacheClearRegExp = /\[simple-cache-mongoose\] CLEAR/;
      const cacheDelRegExp = /\[simple-cache-mongoose\] DEL '.*?'/;

      const userCacheMiss = await UserModel.findById(user._id)
        .cache(ttl, key)
        .exec();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringMatching(cacheMissRegExp)
      );
      ...
      const userCacheHit = await UserModel.findById(user._id)
        .cache(ttl, key)
        .exec();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringMatching(cacheSetRegExp)
      );
      expect(console.log).toHaveBeenCalledWith(
        expect.stringMatching(cacheHitRegExp)
      );
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringMatching(cacheDelRegExp)
      );

      await cache.clear();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringMatching(cacheClearRegExp)
      );
    });
  });
});
